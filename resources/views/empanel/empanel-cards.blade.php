...
<li class="breadcrumb-item">Empanel</li>
<li class="breadcrumb-item active">Empanel List</li>
@endsection
						...
						<table class="display" id="export-button">
							...
							<tbody>
								@php
									$i = 1
								@endphp
								@foreach ($empanel as $article)
								<tr>
									<td>{{ $i++ }}</td>
									<td>{{ ucfirst($article['name']) }}</td>
									<td>{{ $article['email'] }}</td>
									<td>{{ $article['mobile_no'] }}</td>
									<td>{{ ucfirst($article['gender']) }}</td>
