...
class NsdlController extends Controller
{
	...
	public function nsdlResponseUpdate($data, $request) {
		if($data['status'] == "SUCCESS" ) {
			$kyc =  (new KycController)->insertKyc($request->session()->get('id'), "", "", $data['data']['APP_PAN_NO'], "", "", "", "", "approved", $data['data']['APP_STATUS'], 
			"approved", "", "", "", "", "", "", "", "nsdl");
			return "SUCCESS";
		} else {
			if($data['status'] == "FAILURE" ) {
				// if(array_key_exists('ERROR', $data['data'])) {
				// 	$kycURL = (new SignzyController)->signzyGetURL($request);
				// 	if(array_key_exists('ERROR', $kycURL)) {
				// 		return "FAILURE";
				// 	} else {
				// 		$kyc =  (new KycController)->insertKyc($id, "", "", $data['data']['APP_PAN_NO'], "", "", "", "pending", $data['data']['ERROR'], "", $kycURL->createdObj->id, $kycURL->createdObj->username, \Carbon\Carbon::now(), $kycURL->createdObj->autoLoginUrL, $data['data']['ERROR'], "signzy");
				// 		Session::put('url', $kycURL->createdObj->autoLoginUrL);
				// 		return "SUCCESS";
				// 	}
				// } else {
				// 	return "FAILURE";
				// }
				$kyc =  (new KycController)->insertKyc($request->session()->get('id'), "", "", $data['data']['APP_PAN_NO'], "", "", "", "", "approved", $data['data']['ERROR'], "pending", "", "", "", "", "", "", "", "nsdl");
